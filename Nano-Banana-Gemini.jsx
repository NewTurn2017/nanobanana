...
/**
 * Base64 인코딩 - Windows 디버깅 강화 버전
 */
function encodeImageBase64(imageFile, progressWin, isReference, refIndex) {
    try {
        ...
        if (CONFIG.IS_WINDOWS) {
            ...
            if (!copySuccess) {
                // fsName already returns Windows path on Windows
            ...
            // Windows certutil - no path conversion needed on Windows
            var srcPath = fileToEncode.fsName;
            var dstPath = outputFile.fsName;
            
            // certutil command - 수정된 따옴표 처리
            var cmd = 'cmd.exe /c certutil -encode "' + srcPath + '" "' + dstPath + '"';
            
            // certutil 실행 전 파일 크기 확인
            
            // CMD 창 없이 조용히 실행 (실제로는 CMD 창이 뜨)
            executeWindowsCommandSilently(cmd);
            ...
            outputFile = new File(outputFile.fsName); // Refresh
            ...
            if (outputFile.exists) {
                ...
                if (isReference) {
                    try {
                        ...
                        refSuccessFile.writeln("  Base64 length: " + data.length);
                        refSuccessFile.close();
                    } catch(e) {}
                }
        ...
        if (response.indexOf("error") > -1) {
            ...
            try {
                ...
                if (jsonResponse.error) {
                    errorMsg += "오류 코드: " + (jsonResponse.error.code || "알 수 없음") + "\n";
                    errorMsg += "오류 메시지: " + (jsonResponse.error.message || "알 수 없음") + "\n";
                    
                    if (jsonResponse.error.code === 403 || jsonResponse.error.message.indexOf("API key") > -1) {
                        errorMsg += "\nAPI 키를 확인해주세요.";
                    } else if (jsonResponse.error.code === 429) {
...
function testAPIKey(apiKey) {
    try {
        ...
        if (CONFIG.IS_WINDOWS) {
            // CMD 창 없이 조용히 실행
            executeWindowsCommandSilently('cmd.exe /c "' + scriptFile.fsName + '"');
        } else {
...
/**
 * Windows에서 CMD 창 없이 명령 실행
 * 일시적으로 비활성화 - app.system이 CMD 창을 숨기는 기능을 지원하지 않음
 */
function executeWindowsCommandSilently(command) {
    // ExtendScript의 app.system()은 CMD 창을 숨기는 옵션이 없음
    // 어떤 방법을 사용해도 CMD 창이 나타남
    // 추후 CEP 확장으로 변환시 해결 가능
    app.system(command);
    return true;
}
        ...
        try {
            ...
            if (CONFIG.IS_WINDOWS) {
                try {
                    ...
                    // CMD 창 없이 조용히 실행
                    executeWindowsCommandSilently('cmd.exe /c copy /Y "' + srcPath + '" "' + dstPath + '"');
                    allSuccess = true;
                } catch(e2) {
                    allSuccess = false;
                }
